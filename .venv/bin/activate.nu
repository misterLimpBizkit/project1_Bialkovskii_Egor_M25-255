...
export-env {
    ...
    let virtual_env = r#'/Users/limpbizkit/Desktop/project1_Bialkovskii_Egor_M25-255/.venv'#
    let bin = r#'bin'#
    let path_name = if (has-env 'Path') { 'Path' } else { 'PATH' }
    let venv_path = ([$virtual_env $bin] | path join)
    let new_path = ($env | get $path_name | prepend $venv_path)
    let virtual_env_prompt = if (r#'project1-bialkovskii-egor-m25-255-py3.12'# | is-empty) {
        ($virtual_env | path basename)
    } else {
        r#'project1-bialkovskii-egor-m25-255-py3.12'#
    }
